...
import torch ## ADDED BY UDITH
from torchvision import transforms # ADDED BY UDITH
from torchvision import datasets # ADDED BY UDITH
from torch.utils.data import DataLoader # ADDED BY UDITH
import glob # ADDED BY UDITH
import matplotlib.pyplot as plt # ADDED BY UDITH
from PIL import Image, ImageOps # ADDED BY UDITH
import dlutils
...
class get_dataset_celeba(torch.utils.data.Dataset):
    def __init__(self,path='dataset_samples/faces/realign128x128'):
        self.dir_=path
        self.img_list = list(glob.glob(f'{self.dir_}/*.png')) + list(glob.glob(f'{self.dir_}/*.jpg'))
        self.img_size=512
    ...
    def __getitem__(self,idx):
        ...
        img=self.transform_func()(pil_img) # output of this is -> float 0-1
        return img.float().cuda()*255.0 # now the output is 0-255 float (as required)
    def reset(self, lod, batch_size):
        self.img_size = 2 ** lod

def train_celeba(cfg, logger, local_rank, world_size, distributed, log_drive=None):

    #global lod2batch, tracker, sample, samplez, x, model_s, model, encoder_optimizer, decoder_optimizer

    #global lod2batch, tracker, sample, samplez, x, logger, model, cmodel, cfg, encoder_optimizer, decoder_optimizer
    torch.cuda.set_device(local_rank)
    model = Model(
        startf=cfg.MODEL.START_CHANNEL_COUNT,
        layer_count=cfg.MODEL.LAYER_COUNT,
    ...
    #dataset = TFRecordsDataset(cfg, logger, rank=local_rank, world_size=world_size, buffer_size_mb=1024, channels=cfg.MODEL.CHANNELS)
    dataset=get_dataset_celeba(cfg.DATASET.PATH)  ### UDITH ADDED
    ...
    if cfg.DATASET.SAMPLES_PATH != 'no_path':
        ...
        logger.info("ERROR:: UDITH :: NO SAMPLE PATH ******************************************************")
    ...
    for epoch in range(scheduler.start_epoch(), cfg.TRAIN.TRAIN_EPOCHS):
        ...
        dataset.reset(lod2batch.get_lod_power2(), lod2batch.get_per_GPU_batch_size())
        #batches= [torch.randn((lod2batch.get_per_GPU_batch_size(),3, 512, 512))]*3
        #batches = make_dataloader(cfg, logger, dataset, lod2batch.get_per_GPU_batch_size(), local_rank)
        
        ### UDITH
        #my_transform = transforms.Compose([transforms.Resize((512,512)), transforms.ToTensor()])
        logger.info(f"UDITH :: CHANNELS : {cfg.MODEL.CHANNELS}")
        logger.info(f"UDITH :: PATH : {cfg.DATASET.SAMPLES_PATH}")
        data_loader = DataLoader(dataset,batch_size=lod2batch.get_per_GPU_batch_size(),shuffle=True, drop_last=True)
        ### UDITH 
        scheduler.set_batch_size(lod2batch.get_batch_size(), lod2batch.lod)
        logger.disabled = True
        logger.info('UDITH ::: CHECK1')
        model.train()

        need_permute = False
        epoch_start_time = time.time()
        logger.info('UDITH ::: CHECK2')

        i = 0
        #for x_orig in batches:
        for x_orig in tqdm(data_loader):
            logger.info('UDITH ::: CHECK3')
            x_orig= x_orig.cuda(local_rank)
            i += 1
            logger.info('UDITH ::: CHECK4')
            with torch.no_grad():
                logger.info('UDITH ::: CHECK5')
                logger.info(f'UDITH ::: CHECK5 ::: X SHAPE : {x_orig.shape}, GPU BATCH SIZE: {lod2batch.get_per_GPU_batch_size()}, ')
                if x_orig.shape[0] != lod2batch.get_per_GPU_batch_size():
                    logger.info('UDITH ::: CHECK5.0')
                    continue
                logger.info('UDITH ::: CHECK5.1')
                if need_permute:
                    x_orig = x_orig.permute(0, 3, 1, 2)
                x_orig = (x_orig / 127.5 - 1.)
                logger.info('UDITH ::: CHECK5.2')

                blend_factor = lod2batch.get_blend_factor()
                logger.info('UDITH ::: CHECK5.3')
                needed_resolution = layer_to_resolution[lod2batch.lod]
                logger.info('UDITH ::: CHECK5.4')
                x = x_orig

                if lod2batch.in_transition:
                    logger.info('UDITH ::: CHECK5.5')
                    needed_resolution_prev = layer_to_resolution[lod2batch.lod - 1]
                    x_prev = F.avg_pool2d(x_orig, 2, 2)
                    logger.info('UDITH ::: CHECK5.6')
                    x_prev_2x = F.interpolate(x_prev, needed_resolution)
                    x = x * blend_factor + x_prev_2x * (1.0 - blend_factor)
                    logger.info('UDITH ::: CHECK5.7')
            logger.info('UDITH ::: CHECK6')
            x.requires_grad = True
            ...
            encoder_optimizer.step()
            decoder_optimizer.step()
            logger.info('UDITH ::: CHECK8')
            if local_rank == 0:
                betta = 0.5 ** (lod2batch.get_batch_size() / (10 * 1000.0))
                model_s.lerp(model, betta)
            logger.info('UDITH ::: CHECK9')
            epoch_end_time = time.time()
            per_epoch_ptime = epoch_end_time - epoch_start_time

            lod_for_saving_model = lod2batch.lod
            lod2batch.step()
            logger.info('UDITH ::: CHECK10')
            if local_rank == 0:
                logger.info('UDITH ::: CHECK11')
                logger.disabled = False
                if lod2batch.is_time_to_save():
                    checkpointer.save("model_tmp_intermediate_lod%d" % lod_for_saving_model)
                if lod2batch.is_time_to_report():
                    logger.info('UDITH ::: CHECK11.0: SAVING SAMPLE')
                    save_sample(lod2batch, tracker, sample, samplez, x, logger, model_s,
                                model.module if hasattr(model, "module") else model, cfg, encoder_optimizer,
                                decoder_optimizer)
                logger.disabled = True
            logger.info('UDITH ::: CHECK12')
        logger.info('UDITH ::: CHECK13')
        scheduler.step()
        logger.disabled = False
        logger.info(f'training over : epoch {epoch}')
        logger.disabled = True
        if local_rank == 0:
            logger.info('UDITH ::: CHECK13.0')
            checkpointer.save("model_tmp_lod%d" % lod_for_saving_model)
            logger.info('UDITH ::: CHECK13.1')
            save_sample(lod2batch, tracker, sample, samplez, x, logger, model_s,
                        model.module if hasattr(model, "module") else model, cfg, encoder_optimizer, decoder_optimizer)
            logger.info('UDITH ::: CHECK13.2')

        ### log to drive
        if log_drive!=None:log_drive(logger)
    logger.info("Training finish!... save training results")
    if local_rank == 0:
        checkpointer.save("model_final").wait()
...
if __name__ == "__main__":
    ...
    run(train_celeba, get_cfg_defaults(), description='StyleGAN', default_config='configs/ffhq.yaml',
