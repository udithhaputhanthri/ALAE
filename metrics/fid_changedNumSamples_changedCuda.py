...
from launcher_changed import run  #### 4. changed cuda
...
class FID:
    ...
    def evaluate(self, logger, mapping, decoder, model, lod):
        ...
        def compute_for_reals(num_images, path, lod):
            dataset = TFRecordsDataset(self.cfg, logger, rank=1, world_size=1, buffer_size_mb=1024, channels=self.cfg.MODEL.CHANNELS, train=True) #### changed cuda
            dataset.reset(lod + 2, self.minibatch_size)
            batches = make_dataloader(self.cfg, logger, dataset, self.minibatch_size, 1, numpy=True) #### changed cuda 
            activations = []
        ...
        for _ in tqdm(range(0, self.num_images, self.minibatch_size)):
            torch.cuda.set_device(1) #### 3. changed cuda
...
def sample(cfg, logger):
    torch.cuda.set_device(1)  #### 1. changed cuda
    model = Model(
    ...
    model.device_ids=[1]
    model.decoder = nn.DataParallel(decoder, [1])
    with torch.no_grad():
        ppl = FID(cfg, num_images=27500, minibatch_size=16 * torch.cuda.device_count())
